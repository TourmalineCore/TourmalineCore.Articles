...
**Что же выходит с нашим кастомным хуком useClient?**
...
4. Можно безопасно рефакторить код: переставлять местами возвращаемые проперти внутри хука и в местах использования. Ведь порядок больше не важен, в отличие от деструктуризации массивов.
...
**Именованный экспорт, настало твоё время!** Давай использовать тебя всегда.
...
2. Вы не боитесь расширять сигнатуру функций, добавляя новый аргумент. Вам не нужно перелопачивать весь код. И даже новый аргумент со значением по умолчанию не будет проблемой, в отличие от передачи аргументов по одному, когда все аргументы со значением по умолчанию должны идти в конце.
...
5. Кажется (это не точно), при использовании такого подхода IDE лучше работают с переименованием свойств объектов и экспортируемых функций модуля. Хотя VSCode иногда останавливается на полпути, и это не то, что вам нужно.
